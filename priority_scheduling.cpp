...
int burst_time[20],process[20],waiting_time[20],turnaround_time[20],process_priority[20];
...
std::cout<<"Enter priority(coressponds to how small the number is) for process["<<i+1<<"]\t"<<std::endl;
std::cin>>process_priority[i];
process[i]=i+1;
}
for(i=0;i<limit;i++)
{
...
for(j=i+1;j<limit;j++)
{
if(process_priority[j]<process_priority[position])
{
position=j;
}
...
process_priority[i]=process_priority[position];
process_priority[position]=temp;
temp=burst_time[i];
...
std::cout<<"Process["<<process[i]<<"]\t\t"<<process_priority[i]<<"\t\t"<<burst_time[i]<<"\t\t"<<waiting_time[i]<<"\t\t"<<turnaround_time[i]<<std::endl;
}
